...
migrate((app) => {
  if (!isDemoDataEnabled()) {
    console.log("[demo reseed] DEMO_DATA is not TRUE - skipping")
    return
  }
  ...
  const existingDemoAccesses = app.findRecordsByFilter("accesses", "notes = '" + tag + "'", "", 10000, 0)
  for (const rec of existingDemoAccesses) {
    app.delete(rec)
  }
  ...
  console.log("[demo reseed] dashboard demo data refreshed")
  return
}, (app) => {
  const records = app.findRecordsByFilter("accesses", "notes = 'demo_seed_v2'", "", 10000, 0)
